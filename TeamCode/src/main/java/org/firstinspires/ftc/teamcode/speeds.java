package org.firstinspires.ftc.teamcode;

public class speeds {

    public final static double STOP_SPEED = 0.0; //We send this power to the servos when we want them to stop.

    public final static double SLOW_SPEED = 0.2;

    public final static double HALF_SPEED = 0.5;

    public final static double FULL_SPEED = 1.0;

    public final static double LAUNCHER_TARGET_VELOCITY = 1250;
    public final static double LAUNCHER_MIN_VELOCITY = LAUNCHER_TARGET_VELOCITY - 20;

    public final static double DEFAULT_SPEED = 0.7;

    public final static double ENHANCED_SPEED = 0.8;

    public final static double TURN_SPEED = 0.6;     // Max turn speed to limit turn rate.


    public final static double FEED_TIME_SECONDS = 1.00; //The feeder servos run this long when a shot is requested.

}
